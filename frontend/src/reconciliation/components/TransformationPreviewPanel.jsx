...
import { confidenceVariant, CHANGE_BADGE_VARIANT } from "../lib/badgeVariants";

function ConfidenceBadge({ confidence }) {
  if (confidence == null) return null;
  const pct = Math.round(confidence * 100);
  return <Badge variant={confidenceVariant(confidence)}>Confidence: {pct}%</Badge>;
}
        ...
        <tbody>
          {columns.map((c, i) => (
            <tr key={`${c.column}-${i}`}>
              <td>{c.column}</td>
              <td>
                <Badge variant={CHANGE_BADGE_VARIANT[c.change] ?? "default"}>{c.change}</Badge>
              </td>
              <td>{c.from ?? "—"}</td>
            </tr>
          ))}
...
function TransformationPreviewPanel() {
  ...
  return (
    <section
      className={`wizard-section section-shade section-shade--deep${
        scriptApproval ? " is-confirmed" : ""
      }`}
    >
      ...
      {script && (
        <div className="contract-summary">
          <Badge variant="default">Generated By: {script.generated_by}</Badge>
      ...
      {scriptPreview && (
        <>
          <div className="contract-summary">
            <Badge variant="default">{scriptPreview.summary?.text}</Badge>
